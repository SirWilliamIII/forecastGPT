...
if [[ "$NFL_COUNT" == "0" || -z "$NFL_COUNT" ]]; then
    warn "NFL game data not found. Manual setup required:"
    warn "  1. Download CSV datasets (see backend/ingest/backfill_github_nfl.py)"
    warn "  2. Run: cd backend && uv run python -m ingest.backfill_github_nfl"
    warn "  Alternatively, use SportsData.io API for current season:"
    warn "  cd backend && uv run python -m ingest.backfill_sportsdata_nfl --seasons 2024"
    warn ""
    warn "Skipping NFL backfill for now. Backend will start without NFL data."
else
...
echo -e "    • NFL ML model v2.0 (8 teams, 1,699 games)"
echo -e "    • Daily game updates (during season)"
echo -e "    • Team stats & game history"
echo -e "    • Event-based forecasting"
echo -e "${GREEN}══════════════════════════════════════════${NC}"
echo ""
