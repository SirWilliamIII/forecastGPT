...
from config import get_equity_symbols, MAX_DOWNLOAD_RETRIES
...
def main() -> None:
    ...
    for symbol, ticker in equity_symbols.items():
        total_inserted += backfill_equity(symbol, ticker, days=365, horizon_minutes=1440)

    # Update ingest status
    update_ingest_status("equity_backfill", total_inserted)
